...
void guiSplash() {
	...
	if (wifi_en) {
		...
		UG_SetForecolor(C_YELLOW);
		UG_PutString(0, 28, "HTTP://192.168.4.1/");
	} else {
		UG_SetForecolor(C_WHITE);
		UG_PutString(0, 0, "   NOTE:");
		UG_SetForecolor(C_YELLOW);
		UG_PutString(0, 10,  "WiFi is off");
		UG_PutString(0, 18, "and can be ");
		UG_PutString(0, 26, "enabled in ");
		UG_PutString(0, 34, "the options");
		UG_PutString(0, 42, "menu.      ");
	}
	UG_SetForecolor(C_RED);
	UG_PutString(54, 104, "MENU");
